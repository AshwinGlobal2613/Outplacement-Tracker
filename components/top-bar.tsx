...
export function TopBar({ onMenuClick }: TopBarProps) {
  ...
  return (
    <header className="flex h-14 shrink-0 items-center gap-3 border-b border-border bg-background px-4 sm:px-6">
      ...
      <button
        onClick={onMenuClick}
        className="flex items-center justify-center rounded-lg p-1.5 text-muted-foreground hover:bg-sidebar-accent hover:text-foreground transition-colors lg:hidden"
        aria-label="Open menu"
      >
        <Menu className="h-5 w-5" />
      </button>

      {/* Push notification bell + user to the right */}
        ...
        <div ref={ref} className="relative">
          ...
          >
            ...
            <div className="hidden text-left sm:block">
              ...
              <p className="mt-0.5 text-xs text-muted-foreground capitalize">{session?.user?.role === "admin" ? "admin" : "team_member"}</p>
            </div>
          </button>
